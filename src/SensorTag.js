...
class SensorTag extends Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    this.set = new Set();
    this.manufacturerDataSet = new Set();
  }

  componentDidMount() {
    const subscription = this.manager.onStateChange((state) => {
      console.log('componentDidMount onStateChange', state);
        if (state === 'PoweredOn') {
            this.scanAndConnect();
            subscription.remove();
        }
  ...
  scanAndConnect = () => {
    this.manager.startDeviceScan(null, null, (error, device) => {
        ...
        const id = '00:A0:50:03:1F:33';
        const iBeaconManufacturerData = 'TAACFQAFAAEAABAAgAAAgF+bATEABmd4ww==';
        const a = 'AACFQAFAAEAABAAgAAAgF+bATEABmV4ww==';

        // console.log('scanning', device.id === id, device.manufacturerData === iBeaconManufacturerData, device.manufacturerData);
        if (device.manufacturerData && iBeaconManufacturerData.substr(0,16) === device.manufacturerData.substr(0,16)) {
          this.set.add(device.id);
          this.manufacturerDataSet.add(device.manufacturerData);
          console.log(moment(Date.now()).format())
          console.log('iBeacon set size', this.set.size);
          console.log('manufacturerDataSet', this.manufacturerDataSet);
        }
